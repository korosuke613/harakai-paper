...
また、グラフィカルモデルエディタ上にモデルを配置する際、オブジェクト名の確認をポップアップウィンドウで行う。ポップアップウィンドウの例を図\ref{fig:name_ob}に示す。
\begin{figure}[t]
	\centering
...
各部品で使用するMSLを表\ref{tab:MSL}に、ブラシ付きDCモータのModelicaモデルの例を図\ref{fig:tantai_model}に、
...
% 		\end{itemize}
	\end{itemize}
\end{itemize}
以降、モータ特性表を構成する、特性表と特性グラフの内容について述べる。
